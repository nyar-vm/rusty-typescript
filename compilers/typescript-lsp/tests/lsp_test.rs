...
#[tokio::test]
async fn test_extract_symbol_at_position() {
    let service = TypeScriptLanguageService::new(Arc::new(tower_lsp::Client::new()));
...
#[tokio::test]
async fn test_get_line_and_column() {
    let service = TypeScriptLanguageService::new(Arc::new(tower_lsp::Client::new()));
...
#[tokio::test]
async fn test_is_complete_symbol() {
    let service = TypeScriptLanguageService::new(Arc::new(tower_lsp::Client::new()));
...
#[tokio::test]
async fn test_infer_variable_type() {
    let service = TypeScriptLanguageService::new(Arc::new(tower_lsp::Client::new()));
...
#[tokio::test]
async fn test_parse_format_options() {
    let service = TypeScriptLanguageService::new(Arc::new(tower_lsp::Client::new()));

    let options = FormattingOptions { tab_size: 2, insert_spaces: true, ..Default::default() };

    let format_options = service.parse_format_options(&options);
    assert_eq!(format_options.indent_size, 2);
    assert_eq!(format_options.use_tabs, false);
}
