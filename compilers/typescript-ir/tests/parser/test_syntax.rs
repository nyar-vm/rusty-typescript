...
use typescript::{ast::*, builder::TypeScriptBuilder, language::TypeScriptLanguage};
...
fn test_parse_control_flow() {
    ...
    if let Statement::WhileStatement(while_stmt) = &root.statements[1] {
        if let ExpressionKind::BooleanLiteral(val) = &*while_stmt.test.kind {
            assert!(*val)
        }
        else {
            panic!("Expected BooleanLiteral as while test, got {:?}", while_stmt.test)
        }
    }
    else {
...
fn test_parse_expressions() {
    ...
    if let Statement::VariableDeclaration(var) = &root.statements[0] {
        if let Some(expr) = &var.value {
            if let ExpressionKind::ArrowFunction { params, .. } = &*expr.kind {
                assert_eq!(params.len(), 2)
            }
            else {
                panic!("Expected ArrowFunction, got {:?}", var.value)
            }
        }
        else {
    ...
    if let Statement::VariableDeclaration(var) = &root.statements[1] {
        if let Some(expr) = &var.value {
            if let ExpressionKind::ArrayLiteral { elements } = &*expr.kind {
                assert_eq!(elements.len(), 3)
            }
            else {
                panic!("Expected ArrayLiteral, got {:?}", var.value)
            }
        }
        else {
            panic!("Expected value in variable declaration")
    ...
    if let Statement::VariableDeclaration(var) = &root.statements[2] {
        if let Some(expr) = &var.value {
            if let ExpressionKind::ObjectLiteral { properties } = &*expr.kind {
                assert_eq!(properties.len(), 2)
            }
            else {
                panic!("Expected ObjectLiteral, got {:?}", var.value)
            }
        }
        else {
            panic!("Expected value in variable declaration")
